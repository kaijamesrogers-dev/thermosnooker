...
class Simulation:
    ...
    def __init__(self):
        """
        Initialize base Simulation attributes.
        """
...
class MultiBallSimulation(Simulation):
    ...
    def next_collision(self):
        ...
        for i, ball_i in enumerate(self._balls):
            ...
            if t is not None:
                if t < time:
                    time = t
                    # use -1 as sentinel to indicate collision with the container
                    object_colliding = [(i, -1)]
                elif t == time:
                    object_colliding.append((i, -1))
        ...
        for col in object_colliding:
            i0, i1 = col
            if i1 == -1:
                # collision between ball i0 and the container
                self._container.collide(self._balls[i0])
            else:
                # collision between two balls
                self._balls[i0].collide(self._balls[i1])
    ...
    def momentum(self):
        ...
        """
        p_balls_total = np.zeros(2, dtype=float)
        for ball in self._balls:
