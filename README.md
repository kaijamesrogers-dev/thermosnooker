...
### `single_ball_demo` / `multi_ball_animation`

Animated sanity checks. The single-ball case verifies that collision detection and elastic reflection work in the simplest possible setting. The multi-ball animation is a visual check that balls neither stick together nor escape the container.
...
The main result. A tracer particle (radius 2, mass 100) is placed in the gas of light particles and its position tracked over time. The mean squared displacement (MSD) of a Brownian particle grows linearly with time:
...
![MSD vs time](Figures/msd_vs_time.png)

The MSD grows linearly after an initial burn-in period (discarded before fitting, since the system needs time to reach thermal equilibrium).
...
アニメーションによる動作確認。単一球の場合は最も単純な条件で衝突判定と弾性反射を検証する。多球アニメーションは、球同士が重なったり容器から飛び出したりしないことの目視確認である。
...
本プロジェクトの主要な結果。軽い粒子の気体中にトレーサー粒子（半径2、質量100）を置き、その位置を時間追跡する。ブラウン運動する粒子の平均二乗変位（MSD）は時間に比例して増加する:
...
![時間に対するMSD](Figures/msd_vs_time.png)

初期のバーンイン期間（系が熱平衡に達するまでの区間。フィット前に除外）の後、MSDは時間に対して線形に増加する。
